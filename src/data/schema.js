...
/** 中国大陆粗略包围盒，用于识别经纬颠倒等离谱坐标 */
const CHINA = { west: 72, east: 138, south: 3, north: 54 };

/** 点位离城市中心超过这个距离就报警（跨市的点位多半是坐标抄错了） */
...
function inChina({ lng, lat }) {
  return lng >= CHINA.west && lng <= CHINA.east && lat >= CHINA.south && lat <= CHINA.north;
}
...
export function validateCity(city, categories = []) {
  ...
  } else if (!inChina(city.center)) {
    problems.push(warn('city.center', `城市中心 ${city.center.lng},${city.center.lat} 不在中国境内，确认一下`));
  }
  ...
  city.pois.forEach((p, i) => {
    ...
    if (!inChina(p.coord)) {
      ...
      // 交换一下就落回境内，这种情况直接点名，比干巴巴报"超出范围"有用得多
      const swapped = { lng: p.coord.lat, lat: p.coord.lng };
      if (inChina(swapped)) {
        problems.push(err(at(i, 'coord'), `坐标疑似经纬颠倒（写成 lng:${p.coord.lng}, lat:${p.coord.lat}，交换后才落在中国境内）`));
      } else {
        problems.push(err(at(i, 'coord'), `坐标 ${p.coord.lng},${p.coord.lat} 不在中国境内`));
      }
      return;
    }
    if (isCoord(city.center)) {
